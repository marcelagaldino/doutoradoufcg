...
char argv[254];
char *mid[254];
static struct tracepoint * tp_sys_enter_open;
static struct tracepoint * tp_sched_process_exit;
struct rhashtable migration_objects;
...
struct migration_content {
	pid_t pid;
	struct rhash_head head;
};

const static struct rhashtable_params object_params_migration = {
	.key_len	 = sizeof(pid_t),
	.key_offset	 = offsetof(struct migration_content, pid),
	.head_offset = offsetof(struct migration_content, head),
};
