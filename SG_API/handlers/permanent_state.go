...
func PermanentState(w http.ResponseWriter, r *http.Request) {
	...
	if validatePermanentStateEvent(info) {
		sendInfoToModulePermanentStateEvent(info)
	}
}

func sendInfoToModulePermanentStateEvent(info models.PermanentStateEvent) {
...
func validatePermanentStateEvent(op models.PermanentStateEvent) (result bool) {
	if op.VmId == "" || op.Hash == "" {
		return false
	}
	return true
