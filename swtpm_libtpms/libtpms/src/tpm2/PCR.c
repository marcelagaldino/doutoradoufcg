...
int sendVolatileState(char* hash) {
...
//    ret = cbs->tpm_nvram_backend_uri(&name);
    ...
    if(pcrData != NULL)
     ...
     if(hash == 11){
//     if(hash == 11 && handle != 10){
  //   if(hash == 11 && handle == 16){
       char pcr_output[(SHA256_DIGEST_LENGTH * 2) + 1];
       char *ptr = &pcr_output[0];

       for (int i = 0; i < SHA256_DIGEST_LENGTH; i++) {

            ptr += sprintf(ptr, "%02X", pcrData[i]);
//	      ptr += sprintf(ptr, "%02X", data[i]);

        }


        char pcr_output_send[(SHA256_DIGEST_LENGTH * 2) + 2 + 81 + 1];
        strcpy(pcr_output_send, pcr_output);
	...
	if(handle<=9){
		char pcr_number[2];
		sprintf(pcr_number, "%d", handle);
        	strcat(pcr_output_send, pcr_number);
	        strcat(pcr_output_send, "\n");
        ...
        sendVolatileState(pcr_output_send);
