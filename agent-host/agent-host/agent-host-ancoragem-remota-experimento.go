		...
		if management_vm.Uuid != uuid && uuid != "tmp" {
			...
			if err != nil {
				...
				info := permanentState{ID_vm: uuid, Hash: data[0]}
                                ...
                                postBody, _ := json.Marshal(info)
                                requestBody := bytes.NewBuffer(postBody)
			        _, err = http.Post("http://"+management_vm.Address+":"+management_vm.Port+"/ancoragem-vmg-permanent-state", "application/json", requestBody)
                                if err != nil {
                                        fmt.Printf("An Error Occured %v\n", err)
                                }
		...
		if management_vm.Uuid != uuid && uuid != "tmp" {
			...
			if err != nil {
				...
				info := PCR{ID_vm: uuid, Number_PCR: Number_PCR, Hash: data[0]}
                                ...
                                postBody, _ := json.Marshal(info)
                                requestBody := bytes.NewBuffer(postBody)
			        _, err = http.Post("http://"+management_vm.Address+":"+management_vm.Port+"/ancoragem-vmg-volatile-state", "application/json", requestBody)
                                if err != nil {
                                        fmt.Printf("An Error Occured %v\n", err)
                                }
